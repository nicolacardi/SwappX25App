...
//#endregion
  ...
  async onSubmit(){
    ...
    console.log ("reset password richiesto", this.form.controls['Email'].value);
    if(!Utility.validateEmail(this.form.controls['Email'].value)){ 
      this._toast.presentToast ("Inserire una email valida"  );
      return;
    }
    ...
    if (!user) {
      this._toast.presentToast ("Email non registrata");
      return;
    }
    ...
    this.svcMail.inviaMail(objMail).subscribe({
      ...
      error: err=> (this._toast.presentToast( "Errore durante l'invio"))
    });
